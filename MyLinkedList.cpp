...
int MyLinkedList::getAt(int pos) {
    if (pos >= 0 && pos < this->size) { //Domain of the list
        ...
        throw invalid_argument(
                "Couldn't return element in pos " + to_string(pos) + ", list size is " + to_string(size) + ".");
    }
}
...
void MyLinkedList::setAt(int data, int pos) {
    if (pos >= 0 && pos < this->size) { //limits
        ...
        throw invalid_argument(
                "Couldn't set element in pos " + to_string(pos) + ", list size is " + to_string(size) + ".");
    }
}
...
void MyLinkedList::insertFirst(int data) {
    if (this->isEmpty()) {
        // Creates a new Node which holds new data and points nowhere
        this->head = new MyNodeLL(data, nullptr, nullptr);
        this->tail = head;
    } else {
        // Creates a new Node which holds new data and points to the start of the list
        // then saves this node to the head, then points the past of the next node to itself
        this->head = new MyNodeLL(data, this->head, nullptr);
        this->head->next->past = head;
    }
...
void MyLinkedList::insertLast(int data) {
    if (!this->isEmpty()) { // If not empty
        ...
        // The past of the new node points to the tail, and the next to null
        this->tail->next->past = tail;
...
void MyLinkedList::insertAt(int data, int pos) {
    if (pos > 0 && pos < this->size) { // Limits
        ...
        current->next = new MyNodeLL(data, current->next, current);
        current->next->next->past = current->next;
        this->size++; //updates
        ...
        throw invalid_argument(
                "Couldn't insert element in pos " + to_string(pos) + ", list size is " + to_string(size) + ".");
    }
}
...
void MyLinkedList::removeFirst() {
    if (!this->isEmpty()) {
        ...
        if (size == 0) { // assuming head is now null
            this->tail = nullptr; //if no more elements, point tail to null
        } else {
            this->head->past = nullptr; // if head is not null, past of head points to null
        }
...
void MyLinkedList::removeLast() {
    if (this->size <= 1) {
        ...
        MyNodeLL* tmp = this->tail; // Makes tmp point to tail
        this->tail = this->tail->past; // Moves tail to its past
        this->tail->next = nullptr; // Points the next of tail to null
        size--; // Decreases size
...
void MyLinkedList::removeAt(int pos) {
    if (pos > 0 && pos < this->size - 1) { // limits
        ...
        throw invalid_argument(
                "Can't remove element at pos " + to_string(pos) + ", list size is " + to_string(this->size));
    }
}
